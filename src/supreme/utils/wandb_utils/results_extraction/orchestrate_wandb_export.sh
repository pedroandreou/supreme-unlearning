...
# dynamically from: src/supreme/utils/wandb_utils/results_extraction/export_config.py
#
...
CONFIGURATION:
  All values are loaded from: src/supreme/utils/wandb_utils/results_extraction/export_config.py
...
" 2>&1)
			...
			if [ -n "$DICT_NAME" ]; then
				...
				if [ -z "$CLASSES_STR" ]; then
					echo "Error: Could not retrieve class list for dataset '$DATASET' from src/supreme/utils/project_config.py."
					exit 1
				fi
