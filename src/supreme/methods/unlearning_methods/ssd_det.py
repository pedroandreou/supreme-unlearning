"""SSD-Det (Deterministic Selective Synaptic Dampening) unlearning method.
...
The original SSD implementation uses reduction="mean" in the loss, so gradients
are averaged across the batch before squaring. Because squaring is non-linear,
different batch compositions (from shuffling) produce mathematically different
importance scores: mean([a, b])^2 != mean([a, c])^2.

This variant uses reduction="none" and backpropagates each per-sample loss
individually, then squares the per-sample gradient. This computes the sum of
per-sample squared gradients, which is order-independent and matches the paper's
formulation.
"""
...
class ParameterPerturber:
    ...
    def calc_importance(
        ...
        """
        Calculate per-parameter importance using true per-sample squared gradients.

        Unlike the standard SSD calc_importance which computes squared batch-mean
        gradients (order-dependent), this computes Σᵢ gᵢ² by backwarding each
        per-sample loss individually. This is order-independent and matches the
        paper's mathematical formulation.

        Parameters:
        DataLoader (DataLoader): DataLoader to be iterated over
        criterion: loss function with reduction="none" returning per-sample losses
        Returns:
        ...
        for batch_idx, batch in enumerate(dataloader):
            x, _, y = batch
            out = self.model(x)
            losses = criterion(out, y)  # shape: [batch_size], per-sample losses
...
def ssd_det(
    ...
    **kwargs,
    ...
    # Use reduction="none" to get per-sample losses for true per-sample gradient computation
    criterion = nn.CrossEntropyLoss(reduction="none")

    if fabric.global_rank == 0 and wandb_logging_flag:
        config_dict = {
            "optimizer": optimizer.__class__.__name__,
            "loss_function": "CrossEntropyLoss(reduction='none') - per-sample",
            "learning_rate": 0.1,
