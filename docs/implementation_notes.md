...
The framework automatically converts all models to SyncBatchNorm in [unlearn_main.py](../src/supreme/utils/unlearning/unlearn_main.py#L434-L441) before `fabric.setup()`. This conversion must happen before Fabric wraps the model.

**Implementation:** [convert_to_sync_batchnorm()](../src/supreme/utils/fabric/fabric_setup.py#L511-L526)
...
- **Where:** See `src/supreme/methods/unlearning_methods/unsir.py` (`UNSIR_noise.forward` and the call site in `UNSIR_noise_train`).
...
- Parameter-based metrics (layerwise distance) require manual parameter sharding across ranks (see [metrics_main.py:517-598](../src/supreme/eval_metrics/metrics_main.py#L517-L598))
...
DISTRIBUTED_STRATEGY=ddp ./src/supreme/run_local.sh --gpu 0,1 --datasets Cifar100 --models ResNet18
DISTRIBUTED_STRATEGY=fsdp ./src/supreme/run_local.sh --gpu 0,1,2,3 --datasets Cifar100 --models ResNet18
DISTRIBUTED_STRATEGY=deepspeed ./src/supreme/run_local.sh --gpu 0,1 --datasets Cifar100 --models ResNet18

# DeepSpeed with ZeRO Stage 3 (maximum memory savings)
DISTRIBUTED_STRATEGY=deepspeed DEEPSPEED_STAGE=3 ./src/supreme/run_local.sh --gpu 0,1,2,3 --models ResNet18

# SLURM with FSDP
DISTRIBUTED_STRATEGY=fsdp ./src/supreme/run_slurm.sh --gpus 4
```
