...
1. **Push local offline runs to the W&B server** (the `sync` mode - bash, implemented in this script):
   ...
   ```bash
   bash supreme/utils/wandb_utils/runtime/wandb_tools.sh sync                    # basic
   bash supreme/utils/wandb_utils/runtime/wandb_tools.sh sync --delete-synced    # drop local dirs after a successful sync
   bash supreme/utils/wandb_utils/runtime/wandb_tools.sh sync --delete-immediately
   ```
...
2. **Server-side housekeeping** - listing projects, finding/deleting duplicate runs, cleaning empty runs, finding missing seeds, generating duplicate reports. These modes forward to `wandb_manager.py`:
   ...
   ```bash
   bash supreme/utils/wandb_utils/runtime/wandb_tools.sh list-projects
   bash supreme/utils/wandb_utils/runtime/wandb_tools.sh find-duplicates --all
   bash supreme/utils/wandb_utils/runtime/wandb_tools.sh cleanup-empty --project "<NAME>" --delete
   ```
...
```bash
bash supreme/utils/wandb_utils/results_extraction/orchestrate_wandb_export.sh ResNet18 Cifar20 fullclass 32-true 0
```
