...
```bash
# Standalone
DEBUGGER=1 bash src/supreme/run_local.sh --gpu 0 --methods finetune --models ViT --training-seeds 260
...
**Implementation:** attachment in [`src/supreme/utils/training/train_main.py`](../src/supreme/utils/training/train_main.py) and [`src/supreme/utils/unlearning/unlearn_main.py`](../src/supreme/utils/unlearning/unlearn_main.py); core logic in [`src/supreme/utils/debug_utils.py`](../src/supreme/utils/debug_utils.py) (`create_debugger_session()`).
...
When `SCALENE=1`, the framework automatically sets `num_workers=0` for all DataLoaders (see [`src/supreme/utils/generic_utils.py`](../src/supreme/utils/generic_utils.py)) because Scalene has limited multiprocessing support. `SCALENE` can be combined with `DEBUGGER` for simultaneous profiling and debugging.

---

## Fabric callbacks

SUPREME uses [Lightning Fabric callbacks](https://lightning.ai/docs/fabric/2.1.0/guide/callbacks.html) to hook into the training/unlearning/evaluation loops without polluting the main code. Register callback classes in [`src/supreme/utils/fabric/callbacks.py`](../src/supreme/utils/fabric/callbacks.py) and call their hooks:
...
When callbacks are enabled, the framework logs every 10 batches (see `if batch_idx % 10 == 0:` in [`src/supreme/utils/fabric/callbacks.py`](../src/supreme/utils/fabric/callbacks.py)). The regular logging also feeds the [process tracker](#process-auto-cleanup) so it can distinguish healthy progress from stalls.
...
- `class_distribution/` - `forget_class_distribution.csv` and `retain_class_distribution.csv` with columns `Class Type, Class Name, Count`:
  ...
  ...
  ```
  `Unknown Class` means the integer label wasn't registered in [`src/supreme/utils/project_config.py`](../src/supreme/utils/project_config.py) (e.g. `cifar20_dict`, `cifar100_dict`, `pins_dict`) - we only define labels for classes we care about for forgetting. Reference: original mappings in [bad-teaching-unlearning](https://github.com/vikram2000b/bad-teaching-unlearning).
...
Useful for verifying splits, catching data leakage, and debugging unexpected per-class results. Implementation in [`src/supreme/utils/debug_utils.py`](../src/supreme/utils/debug_utils.py).
...
./src/supreme/utils/wandb_utils/results_extraction/orchestrate_wandb_export.sh --all-existing

# Single combination
./src/supreme/utils/wandb_utils/results_extraction/orchestrate_wandb_export.sh --export ResNet18 Cifar20 fullclass veg

# Clean generated files
./src/supreme/utils/wandb_utils/results_extraction/orchestrate_wandb_export.sh --clean
```
...
- [`src/supreme/utils/wandb_utils/results_analysis/pins_paper_tables.ipynb`](../src/supreme/utils/wandb_utils/results_analysis/pins_paper_tables.ipynb) - produces the exact LaTeX tables in the paper (Pins Face Recognition main + appendix).
