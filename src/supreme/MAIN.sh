...
find_checkpoint() {
	local net_type=$1
	local dataset=$2
	python "${root_dir}/src/supreme/utils/unlearning/update_checkpoint_paths.py" \
...
train_model() {
	...
	local train_cmd="${PYTHON_LAUNCHER} ${root_dir}/src/supreme/utils/training/train_main.py \
        -unlearning_seed \"$TRAINING_SEED\" \
        -precision \"$training_precision\" \
        -net \"$model_type\" \
        -dataset \"$dataset\" \
...
run_unlearning_method() {
	...
	local forget_script="${root_dir}/src/supreme/utils/unlearning/unlearn_main.py"
...
run_evaluation() {
	...
	if [ "$force_reevaluation" = false ]; then
		local check_script="${root_dir}/src/supreme/utils/wandb_utils/runtime/wandb_setup.py"
