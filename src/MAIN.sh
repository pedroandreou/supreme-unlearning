...
root_dir=$(realpath "${script_dir}/..")  # script_dir = src/ → .. = project root
...
find_checkpoint() {
    local net_type=$1
    local dataset=$2
    python "${root_dir}/src/utils/unlearning/update_checkpoint_paths.py" \
...
train_model() {
    ...
    local train_cmd="${PYTHON_LAUNCHER} ${root_dir}/src/utils/training/train_main.py \
        -unlearning_seed \"$TRAINING_SEED\" \
        -precision \"$training_precision\" \
        -net \"$model_type\" \
        -dataset \"$dataset\" \
...
run_unlearning_method() {
    ...
    local forget_script="${root_dir}/src/utils/unlearning/unlearn_main.py"
...
run_evaluation() {
    ...
    local eval_script="${root_dir}/src/utils/unlearning/unlearn_main.py"
    ...
    if [ "$force_reevaluation" = false ]; then
        local check_script="${root_dir}/src/utils/wandb_utils/runtime/wandb_setup.py"
...
for j in "${UNLEARNING_SEEDS[@]}"; do
    ...
    needs_mr=false
    if [[ $(python3 -c "import sys; sys.path.append('${root_dir}'); from src.utils.unlearning.unlearn_main import requires_retrain; print(requires_retrain('${EVAL_METRICS}'.split(',')))" 2>/dev/null) == "True" ]]; then
        needs_mr=true
    fi
