...
# dynamically from: src/utils/wandb_utils/results_extraction/export_config.py
#
...
get_seeds() {
	python -c "
from src.utils.wandb_utils.results_extraction.export_config import SEEDS
...
get_default_prefix() {
	python -c "
from src.utils.wandb_utils.results_extraction.export_config import DEFAULT_EXPERIMENT_PREFIX
...
get_default_precision() {
	python -c "
from src.utils.wandb_utils.results_extraction.export_config import DEFAULT_PRECISION
...
get_models() {
	python -c "
from src.utils.wandb_utils.results_extraction.export_config import MODELS
print(' '.join(MODELS))
"
}
...
get_datasets() {
	python -c "
from src.utils.wandb_utils.results_extraction.export_config import DATASETS
print(' '.join(DATASETS))
"
}
...
get_prefixes() {
	python -c "
from src.utils.wandb_utils.results_extraction.export_config import EXPERIMENT_PREFIXES
...
CONFIGURATION:
  All values are loaded from: src/utils/wandb_utils/results_extraction/export_config.py
...
run_specific_combination() {
	...
	if [ "$strategy" == "random_" ]; then
		...
		python -m src.utils.wandb_utils.results_extraction.wandb_metrics_exporter --export \
			-model "$model" \
			-dataset "$dataset" \
...
from src.utils.wandb_utils.results_extraction.export_config import DATASET_CLASSES
...
" 2>&1)
			...
			DICT_NAME=$(python -c "
import src.utils.project_config as project_config
try:
...
" 2>&1)
			...
			if [ -n "$DICT_NAME" ]; then
				CLASSES_STR=$(python -c "import src.utils.project_config as project_config; print(' '.join(getattr(project_config, '$DICT_NAME').keys()))" 2>/dev/null)
				if [ -z "$CLASSES_STR" ]; then
					echo "Error: Could not retrieve class list for dataset '$DATASET' from src/utils/project_config.py."
					exit 1
				fi
...
from src.utils.wandb_utils.results_extraction.export_config import EXISTING_PROJECTS
...
from src.utils.wandb_utils.results_extraction.export_config import DATASET_STRATEGIES
...
if [ "$COMBINE_ONLY" = true ] || [ "$ALL_STEPS" = true ]; then
	...
	python -m src.utils.wandb_utils.results_extraction.wandb_metrics_exporter --combine \
		--prefix "$EXPERIMENT_PREFIX" \
